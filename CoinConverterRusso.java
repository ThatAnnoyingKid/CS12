...
public class CoinConverterRusso {
  ...
  public static void convert(float yuan) // This method does the calculations for us
    ...
    System.out.println(
        (int) yuan + " Is equal to " + pennies); // We assume user input here just becuase I can
    String[] labels = {
      "dollars", "quarters", "dimes", "nickles", "pennies"
    }; // This is a neat trickaroo where if we assume largest to smallest it will convert coins
    int[] coins = {
      100, 25, 10, 5, 1
    }; // We do the same trickaroo from above but this time we define the values
    for (int i = 0; i < labels.length; i++) {
      System.out.println(
          (pennies / coins[i])
              + " "
              + labels[i]); // Here it will combine lines 28 and 29 to output the value and coins
