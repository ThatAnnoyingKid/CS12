...
public class GallonConversionRusso {
  ...
  public static void description() {
    ...
    for (int i = 0; i < 5; i++) System.out.println("&" + centerString(i == 1 || i == 2 ? inputs[i - 1] : "", CONSOLE_WIDTH - 2, i == 0 || i == 4 ? '&' : ' ') + "&");
  }
  ...
  public static void conversion() {
    ...
    for (int i = 1; i < 100; i += 4){
		double liters;
		liters = MULTIPLIER * i;
		System.out.println(centerString(rightPad("" + i, 7) + "    " + rightPad("" + liters, 18), CONSOLE_WIDTH, ' ')); // This works by multpling the gallons by the constant multpler
	}
  }
  ...
  public static String rightPad(String input, int width) {
    return input + (input.length() < width ? " ".repeat(width - input.length()) : ""); // may i have forgiveness
  }
}
